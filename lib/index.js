...
exports.expect = function (value) {
    ...
    return new internals.Assertion(value);
};
...
internals.Assertion.prototype.assert = function (expr, msg, actual, expected) {
    ...
    message = message.replace(/#{act}/g, internals.display(actual)).replace(/#{exp}/g, internals.display(expected));
...
[true, false, null, undefined].forEach(function (value) {
    ...
    internals.addMethod(name, function () {
        ...
        this.assert(this._ref === value, 'be ' + name);
    });
});
...
internals.addMethod('empty', function () {
    ...
    Hoek.assert(typeof this._ref === 'object' || this._ref === 'string', 'Can only assert empty on object or string');
...
internals.addMethod(['equal', 'equals'], function (value) {
    ...
    this.assert(compare(this._ref, value), 'equal ' + internals.display(value), this._ref, value);
});


internals.addMethod(['above', 'greaterThan'], function (value) {

    this.assert(this._ref > value, 'be above ' + value);
});


internals.addMethod(['least'], function (value) {

    this.assert(this._ref >= value, 'be at least ' + value);
});


internals.addMethod(['below', 'lessThan'], function (value) {

    this.assert(this._ref < value, 'be below ' + value);
});


internals.addMethod(['most'], function (value) {

    this.assert(this._ref <= value, 'be at most ' + value);
});


internals.addMethod('within', function (start, finish) {

    this.assert(this._ref >= start && this._ref <= finish, 'be within ' + start + '..' + finish);
});
...
internals.addMethod('about', function (value, delta) {
    ...
    this.assert(Math.abs(this._ref - value) <= delta, 'be about ' + value + ' \u00b1' + delta);
});
...
internals.addMethod(['throw', 'throws'], function (/* [constructor,] message */) {
    ...
    var constructor = arguments.length && typeof arguments[0] !== 'string' && !(lastArg instanceof RegExp) ? arguments[0] : null;
    ...
    try {
        ...
        if (message) {
            this.assert(err.message && err.message.match(message), 'throw error matching "' + message + '"', err.message);
        }
    }

    this.assert(thrown, 'throw');
});
