...
internals.match = function (regex) {
    ...
    if (this._ref instanceof Array) {

        return this.assert(internals.matchArray(this._ref, regex), 'match ' + regex);
    }
    return this.assert(regex.exec(this._ref), 'match ' + regex);
};

internals.matchArray = function (items, regex) {

    return items.some((item) =>

        regex.exec(item) !== null
    );
